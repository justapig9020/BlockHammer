...
bool Processor::any_core_finished()
{
  for (unsigned int i = 0; i < cores.size(); i++)
  {
    if (cores[i]->finished())
...
bool Processor::every_core_finished()
{
  for (unsigned int i = 0; i < cores.size(); i++)
  {
    if (!cores[i]->finished())
...
bool Processor::finished()
{
  ...
  if (early_exit)
    finished = any_core_finished();
  else
    finished = every_core_finished();
  if (finished)
    update_ipc();
  return finished;
}
...
void Core::tick()
{
  ...
  if (reached_limit && rest_in_peace)
    return;
