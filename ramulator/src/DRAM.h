...
namespace ramulator
{
    ...
    // Constructor
    template <typename T>
    DRAM<T>::DRAM(T *spec, const Config &configs, typename T::Level level, int id, const string identifier) : spec(spec), level(level), id(0), parent(NULL), configs(configs)
    {
        ...
        if (identifier != "")
            this->identifier_str = identifier + "_" + to_string(id);
        else
            this->identifier_str = to_string(id);
    ...
    template <typename T>
    void DRAM<T>::tick(const long clk)
    {
        if (rhdef)
            rhdef->tick(clk);
    }
