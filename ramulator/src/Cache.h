...
namespace ramulator
{
  ...
  public:
    ...
    struct Line
    {
      ...
      Line(long addr, long tag) : addr(addr), tag(tag), lock(true), dirty(false), is_prefetch(false) {}
      Line(long addr, long tag, bool lock, bool dirty) : addr(addr), tag(tag), lock(lock), dirty(dirty), is_prefetch(false) {}
    };
  ...
  protected:
    ...
    void update_cache_access_counters(Request &req);

    bool do_cache_hit(std::list<Line> &lines, std::list<Line>::iterator &line, Request &req);

    bool do_cache_miss(std::list<Line> &lines, std::list<Line>::iterator &line, Request &req);

    bool mshr_forwarding(Line &mshr_line, Request &req, bool is_dirty);
  ...
  public:
    CacheSystem(const Config &configs, std::function<bool(Request)> send_memory,
                std::function<bool(long)> upgrade_prefetch_req) : send_memory(send_memory), upgrade_prefetch_req_in_mem(upgrade_prefetch_req), throttling_coeffs(throttling_coeffs)
    {
